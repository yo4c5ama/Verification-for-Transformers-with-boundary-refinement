...
class Bounds:
    ...
    def add_linear(self, mask, w_out, b_out, type1, k, x0, y0, src=None):
        if mask is None: 
            mask_w = mask_b = 1
        else:
    ...
    def get_bounds_xy(self, l_x, u_x, l_y, u_y, debug=False, z = False):
        if self.ibp:
            prod1 = l_x * l_y
            prod2 = l_x * u_y
            prod3 = u_x * l_y
    ...
    def get_lubounds_xy(self, l_x, u_x, l_y, u_y, debug=False, z = True, lbounds = True):
    ...
    def dot_product_wrong(self, other, debug=False, verbose=False, lower=True, upper=True):
        if self.dim_in == 1:
        ...
        # l_a, u_a, l_b, u_b = [], [], [], []
        l_left_a, l_right_a, u_left_a, u_right_a = self.attention_bound_concretize()
        l_left_b, l_right_b, u_left_b, u_right_b = other.attention_bound_concretize()
        # l_a, u_a = self.concretize()
        # l_b, u_b = other.concretize()
        def _propagate(l_a, u_a, l_b, u_b, lbounds=True):
            w = torch.zeros(self.lw.shape[0], self.lw.shape[1], self.dim_in, other.lw.shape[1]).to(self.device)
            b = torch.zeros(self.lw.shape[0], self.lw.shape[1], other.lw.shape[1]).to(self.device)

            for t in range(self.batch_size):
                alpha, beta, gamma = \
                    self.get_lubounds_xy(
                ...
                alpha = alpha.reshape(self.length, other.length, self.dim_out)
                beta = beta.reshape(self.length, other.length, self.dim_out)
                gamma = gamma.reshape(self.length, other.length, self.dim_out)


                b[t] += torch.sum(gamma, dim=-1)


                matmul_batch_size = 128  # 64

                def add_w_alpha(new, old, weight):
                ...
                def add_b_alpha(new, old, weight):
                    new[t, :, :] += \
                ...
                if lbounds:
                    add_w_alpha(w, self.lw, alpha)
                    add_w_beta(w, other.lw, beta)
                    add_b_alpha(b, self.lb, alpha)
                    add_b_beta(b, other.lb, beta)
                else:
                    add_w_alpha(w, self.uw, alpha)
                    add_w_beta(w, other.uw, beta)
                    add_b_alpha(b, self.ub, alpha)
                    add_b_beta(b, other.ub, beta)
            return w, b

        lw,lb = _propagate(l_left_a, l_right_a, l_left_b, l_right_b)
        uw,ub = _propagate(u_left_a, u_right_a, u_left_b, u_right_b,lbounds=False)
        return Bounds(
            self.args, self.p, self.eps,
            lw=lw, lb=lb, uw=uw, ub=ub
        )
    ...
    def dot_product_double(self, other, debug=False, verbose=False, lower=True, upper=True):
        if self.dim_in == 1:
            l1, u1 = self.lb.unsqueeze(-2), self.ub.unsqueeze(-2)
            l2, u2 = other.lb.unsqueeze(1), other.ub.unsqueeze(1)
            prod1, prod2, prod3, prod4 = l1 * l2, l1 * u2, u1 * l2, u1 * u2
        ...
        def _propagate(double_z = False):
            ...
            bounds = Bounds(self.args,  self.p, self.eps, lw = lw, lb = lb, uw = uw, ub = ub)
            return bounds
    ...
    def context(self, value):
        if self.double_z:
            context1, context2 = self.dot_product_double(value.t())
